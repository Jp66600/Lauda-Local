...
Diferente do `vellum ui` (que abre uma página no navegador), aqui a
...
log = logging.getLogger("vellum.desktop")
...
class VellumApp:
    def __init__(self, root: Tk, theme_choice: str | None = None) -> None:
        ...
        app_logger = logging.getLogger("vellum")
    ...
    def check_ollama(self) -> None:
        ...
        threading.Thread(target=trabalho, name="vellum-ollama", daemon=True).start()
    ...
    def download_ollama_model(self) -> None:
        ...
        threading.Thread(target=trabalho, name="vellum-pull", daemon=True).start()
    ...
    def _build_perf_summary(self, page: tk.Frame) -> None:
        ...
        page.columnconfigure(0, weight=1)
        ttk.Label(
            page, text="Como o Vellum vai usar este computador", style="Section.TLabel"
    ...
    def _grab_thumbnail(self, path: Path) -> Any:
        ...
        try:
            tools = resolve_tools()
            with tempfile.TemporaryDirectory(prefix="vellum-capa-") as tmp:
    ...
    def check_machine(self) -> None:
        ...
        threading.Thread(target=trabalho, name="vellum-hardware", daemon=True).start()
    ...
    def shutdown(self) -> None:
        logging.getLogger("vellum").removeHandler(self.log_handler)
...
def main(theme: str | None = None) -> int:
    """Ponto de entrada do aplicativo de janela (`vellum-app`).
    ...
    root = Tk()
    app = VellumApp(root, theme_choice=theme)
