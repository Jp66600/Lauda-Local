...
Uso (interno):
    ...
    python -m vellum.worker caminho/do/job.json
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv:
        emit({"t": EVENT_ERROR, "message": "uso: python -m vellum.worker job.json"})
        return 2
    ...
    try:
        ...
        logging.getLogger("vellum.worker").debug("stdout sem UTF-8 (%s).", exc)
