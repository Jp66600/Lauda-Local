...
Tudo que é do usuário mora em `~/.lauda`: preferências, histórico, pontos de
...
O programa já se chamou **MediaIntel Local** e depois **Vellum**, e quem testou
aquelas versões tem um `~/.mediaintel` ou um `~/.vellum` cheio de escolhas.
Trocar o nome não pode significar começar do zero, então na primeira abertura o
perfil antigo é **copiado** para o novo.
...
log = logging.getLogger("lauda.profile")

#: Onde ficam preferências, histórico, pontos de retomada e logs.
DATA_DIR = Path.home() / ".lauda"

#: Nomes anteriores do produto, do mais recente para o mais antigo. A ordem
#: importa: quem passou por Vellum tem lá o perfil mais atual.
LEGACY_DIRS: tuple[Path, ...] = (
    Path.home() / ".vellum",
    Path.home() / ".mediaintel",
)
