...
Alem da saida na tela, tudo vai para um arquivo em `~/.lauda/logs`. O
...
LOG_DIR = Path(os.environ.get("LAUDA_LOG_DIR", Path.home() / ".lauda" / "logs"))
...
def setup_logging(
    ...
    logging.getLogger("lauda").setLevel(min(level, logging.DEBUG))
    quiet_third_party(verbose=verbose)
    return logging.getLogger("lauda")
...
def log_uncaught(logger: logging.Logger | None = None) -> None:
    ...
    """
    log = logger or logging.getLogger("lauda")
