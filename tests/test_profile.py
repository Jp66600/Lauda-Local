...
perder tema, limites, opções e histórico só porque o produto virou Vellum.
"""
...
from vellum.profile import migrate_legacy_profile
...
def test_a_pasta_antiga_continua_intacta(tmp_path: Path):
    ...
    migrate_legacy_profile(tmp_path / ".vellum", (antiga,))
...
def test_pasta_nova_criada_pelo_log_nao_bloqueia_a_migracao(tmp_path: Path):
    ...
    (nova / "logs" / "vellum.log").write_text("linha", encoding="utf-8")
...
def test_sem_perfil_antigo_nao_faz_nada(tmp_path: Path):
    assert migrate_legacy_profile(tmp_path / ".vellum", (tmp_path / ".mediaintel",)) is None
    assert not (tmp_path / ".vellum").exists()
...
def test_falha_de_escrita_nao_derruba_a_abertura(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("vellum.profile.shutil.copy2", explode)

    assert migrate_legacy_profile(tmp_path / ".vellum", (antiga,)) is None
...
def test_no_pacote_os_modelos_ficam_no_perfil(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(profile, "DATA_DIR", tmp_path / ".vellum")
    monkeypatch.setattr(_sys, "frozen", True, raising=False)

    assert profile.models_dir() == tmp_path / ".vellum" / "models"
