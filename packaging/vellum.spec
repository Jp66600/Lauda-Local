...
"""Receita do PyInstaller para o aplicativo de janela.
    ...
    .venv-build\\Scripts\\pyinstaller.exe packaging\\vellum.spec --noconfirm

Gera `dist/Vellum/` — uma pasta que roda em qualquer Windows 64 bits,
...
dados += collect_data_files("vellum", includes=["resources/**"])
...
dados.append((str(RAIZ / "assets" / "vellum.ico"), "assets"))
...
analise = Analysis(
    ...
    binaries=binarios,
    datas=dados,
    hiddenimports=ocultos + ["vellum.desktop"],
    hookspath=[],
    runtime_hooks=[],
...
exe = EXE(
    ...
    icon=str(RAIZ / "assets" / "vellum.ico"),
)
